...
export default function Img2Img() {
  ...
  const previewImage = `/sdapi/task/read?name=${image}`
  ...
  const uploadFile = async (files: FileList | null) => {
    if (files) {
      ...
      const res = await fetch('/sdapi/task/upload', {
        method: 'POST',
        body: formData,
      }).then(res => res.json())
      setImage(res.dest)
  ...
  const handleSubmit = async () => {
    setSubmiting(true)
    const res = await fetch('/sdapi/task/img2Image', {
      method: 'POST',
      headers: {
    ...
    <Box sx={{ display: 'flex' }}>
      <Paper className='flex-1' sx={{ padding: '0 24px', width: '100%', height: '100%', overflow: 'auto' }}>
        ...
        <Box sx={{ padding: '24px 0', display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <Box>
            <p className='mb-3 text-[18px]'>上传图片：</p>
            <label
              htmlFor='img2img-file'
              className='flex items-center justify-center w-[100px] h-[100px] border-dashed border-[1px] rounded-[6px] cursor-pointer overflow-hidden'>
              {
                image ? (
                  <img src={previewImage} className='w-full h-full object-cover'/>
                ) : <Add />
              }
              <input className='w-0 h-0 overflow-hidden invisible' id='img2img-file' type="file" onChange={(e) => uploadFile(e.target.files)} />
            </label>
            <p>{ image }</p>
          </Box>
          {
